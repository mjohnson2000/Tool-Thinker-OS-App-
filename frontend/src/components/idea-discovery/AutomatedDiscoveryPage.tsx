...
export function AutomatedDiscoveryPage() {
  ...
  React.useEffect(() => {
    ...
    fetchFeedback();
  }, [currentStage, personas.length, businessIdea, customerDescription, id]);
        ...
        {STAGES.map((stage, idx) => (
          <div 
            ...
            style={{ 
              ...
              alignItems: 'center',
              cursor: idx <= currentStage ? 'pointer' : 'default',
              opacity: idx <= currentStage ? 1 : 0.5,
              transition: 'all 0.2s ease'
            }}
            onClick={() => {
              // Allow navigation to any stage that has been reached or is current
              if (idx <= currentStage) {
            ...
            title={idx <= currentStage ? `Go to ${stage}` : `${stage} (not yet available)`}
          >
            <div style={{
              width: 28,
              height: 28,
            ...
            <span style={{ 
              fontSize: 12, 
              color: idx === currentStage ? '#181a1b' : idx < currentStage ? '#666' : '#888', 
              textAlign: 'center', 
              maxWidth: 80,
